//
// Created by Карим Вафин on 13.03.2022.
//
...
public:
    ...
    void move(double tau, double& time) {
        x += vx * tau;
        y += vy * tau;
        z += vz * tau;

        smth = 4000 * std::sin(6.28 * time + 0.5 * z);

        if (y > 0 && x > -50 && x < -20 && z > 24)
            smth = 4000;
        ...
        if (y > 0 && x > -50 && x < -20 && z > 24)
        {
            ...
            if (change)
            {
                vx = - 5 * (x + 50) * y / 20 / 25 * 0.34;
                vy = 5 * (x + 50) * y / 20 / 25 * 0.94;
                vz = 5 * (x + 50) * y / 20 / 25 * std::sin(6.28 * time);
            }
            else
            {
                vx = 5 * (x + 50) * y / 20 / 25 * 0.34;
...
public:
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "serpent-step-" + std::to_string(snap_number) + ".vtu";
...
int main()
{
    ...
    gmsh::initialize();
    gmsh::model::add("serpent");

    // Считаем STL
    try {
        gmsh::merge("/Users/karimvafin/Documents/Repositories/MIPT/miniscience/lab_gmsh/serpent.stl");
    } catch(...) {
    ...
    gmsh::model::mesh::field::setString(f, "F", "200");
